...
registerViz({
  ...
  generate(spec: VizSpec, ctx: VizContext) {
    ...
    items.forEach((item, i) => {
      ...
      radialLabel(ctx, cx, cy, D / 2, mid, `${pct}% ${item.label}`, item.detail, role.color, { maxW: 190 });
      if (item.icon) {
...
registerViz({
  ...
  generate(spec: VizSpec, ctx: VizContext) {
    ...
    ctx.label(`${Math.round(value)}%`, cx, cy + 58, { size: 27, color: role.color, weight: 700, font: "heading" });
...
registerViz({
  ...
  generate(spec: VizSpec, ctx: VizContext) {
    ...
    const cause = spec.items.find((i) => i.kind === "cause" || i.kind === "center") ?? { label: spec.title ?? "Cause", detail: undefined, icon: undefined, color: undefined, id: "cause" };
    const n = Math.max(items.length, 1);
    const cx = 360;
    const cy = 360;
    const causeR = 86;
...
registerViz({
  ...
  generate(spec: VizSpec, ctx: VizContext) {
    ...
    items.forEach((item, i) => {
      ...
      ctx.label(`${Math.round(value)}%`, cxP, gy + D + 32, { size: 24, color: role.color, weight: 700, font: "heading" });
    });
    if (summary) {
      const b = ctx.bounds();
